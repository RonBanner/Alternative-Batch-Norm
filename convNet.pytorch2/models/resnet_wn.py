...
from models.swn import weight_norm as wn
from .mean_bn import MeanBN


# from .own import orth_weight_norm as wn
...
def init_model(model):
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            ...
            # m.bias.data.zero_()
            # Ron Change two new lines of code
            m.mean.data.zero_()
            m.scale.data.fill_(1)
...
class ResNet_WN_imagenet(ResNet_WN):
    ...
    def __init__(self, num_classes=1000,
        ...
        self.fc = wn(nn.Linear(512 * block.expansion,
                               num_classes), p=p, noise_std=noise_std)
        ...
        self.regime = [
            ...
            {'epoch': 60, 'lr': 1e-2},
            {'epoch': 75, 'lr': 1e-3, 'weight_decay': 0},
            {'epoch': 95, 'lr': 1e-4}
        ]
