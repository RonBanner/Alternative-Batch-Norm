...
model_names.append('resnet_NoWn')
...
def main():
    ...
    ## HERE - START
    # model.fp32_params = {n: p.clone().cuda().float().detach()
    #                      for (n, p) in model.named_parameters()}
    #
    # for p in model.fp32_params.values():
    #     p.requires_grad = True
    # optimizer = OptimRegime(model.fp32_params.values(), regime)
    ## HERE - END
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if training:
            ...
            for (n, p) in model.named_parameters():
                if args.gpus and len(args.gpus) > 1:
                    n = n.replace('module.', '')
                fp32_params[n].backward(p.grad.float())
            optimizer.step()
            for n, p in model.named_parameters():
                # n = n.replace('module.', '')
                if args.gpus and len(args.gpus) > 1:
                    n = n.replace('module.', '')
                p.data.copy_(fp32_params[n].data)
